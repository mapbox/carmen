...
(() => {
    ...
    let coords = [
        ...
        [-79.3770521879196, 38.83398302448309],
        ...
        [-79.37780320644379, 38.8337531956001]
    ];
    ...
    tape('default response is 1 features (reverse)', t => {
        c.geocode('-79.37745451927184,38.83420867393712', {}, (err, res) => {
            ...
            t.equal(
                ...
                'seneca rocks 5, main road, west virginia'
            );
    ...
    tape('Limit only works with type (reverse)', t => {
        c.geocode(
            '-79.37745451927184,38.83420867393712',
            { limit: 2 },
            (err, res) => {
                t.ok(err);
                t.end();
    ...
    tape('limit 5 results (reverse)', t => {
        c.geocode(
            ...
            (err, res) => {
                ...
                t.equal(
                    res.features[1].place_name,
                    'seneca rocks 2, main road, west virginia'
                );
                t.equal(
                    res.features[2].place_name,
                    'seneca rocks 3, main road, west virginia'
                );
                t.equal(
                    res.features[3].place_name,
                    'seneca rocks 4, main road, west virginia'
                );
                t.equal(
                    res.features[4].place_name,
                    'seneca rocks 1, main road, west virginia'
                );
                t.equal(res.features.length, 5, 'returns 5 results');
                t.end();
            }
    ...
    tape('limit 5 results (address)', t => {
        c.geocode(
            ...
            (err, res) => {
                ...
                t.equal(
                    res.features[0].place_name,
                    '5 main road, west virginia'
                );
                t.equal(
                    res.features[1].place_name,
                    '6 main road, west virginia'
                );
                t.equal(
                    res.features[2].place_name,
                    '2 main road, west virginia'
                );
                t.equal(
                    res.features[3].place_name,
                    '3 main road, west virginia'
                );
                t.equal(
                    res.features[4].place_name,
                    '8 main road, west virginia'
                );
                t.equal(
                    res.features.length,
                    5,
    ...
    tape('Reverse Cluster', t => {
        c.geocode(
            ...
            (err, res) => {
                ...
                t.equal(
                    res.features[1].place_name,
                    '6 main street, west virginia'
                );
                t.equal(
                    res.features[2].place_name,
                    '2 main street, west virginia'
                );
                t.equal(
                    res.features[3].place_name,
                    '3 main street, west virginia'
                );
                t.equal(
                    res.features[4].place_name,
                    '8 main street, west virginia'
                );
                t.equal(
                    res.features.length,
                    5,
...
(() => {
        ...
        queueFeature(
            conf.address,
            {
                ...
                geometry: {
                    ...
                    coordinates: [
                        [
                            [-79.378382563591, 38.83475190117003],
                            [-79.37798023223877, 38.83472265057851]
                        ],
                        [
                            [-79.37795341014862, 38.83472265057851],
                            [-79.37729358673094, 38.834651613377574]
                        ],
                        [
                            [-79.37725603580475, 38.83463907739358],
                            [-79.3767088651657, 38.834593112100016]
                        ],
                        [
                            [-79.37676787376404, 38.834430144001914],
                            [-79.37698245048523, 38.833962130978954]
                        ]
                    ]
                }
            },
    ...
    tape('Reverse ITP', t => {
        c.geocode(
            '-79.37745451927184,38.83420867393712',
            { limit: 3, types: ['address'] },
            (err, res) => {
                t.equal(
                    res.features[0].place_name,
                    '13 main street, west virginia'
                );
                t.equal(
                    res.features[1].place_name,
                    '10 main street, west virginia'
                );
                t.equal(
                    res.features[2].place_name,
                    '6 main street, west virginia'
