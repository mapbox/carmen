...
tape('error: invalid options.proximity type', t => {
    c.geocode('province', { proximity: 'adsf' }, (err, res) => {
        t.equal(
            err && err.toString(),
            'Error: Proximity must be an array in the form [lon, lat]'
        );
...
tape('error: invalid options.proximity[0] type', t => {
    c.geocode('province', { proximity: [{}, 0] }, (err, res) => {
        t.equal(
            err && err.toString(),
            'Error: Proximity lon value must be a number between -180 and 180'
        );
        t.equal(err && err.code, 'EINVALID');
        t.end();
    });
...
tape('error: invalid options.proximity[1] type', t => {
    c.geocode('province', { proximity: [0, {}] }, (err, res) => {
        t.equal(
            err && err.toString(),
            'Error: Proximity lat value must be a number between -90 and 90'
        );
        t.equal(err && err.code, 'EINVALID');
        t.end();
    });
...
tape('forward country - proximity - single layer - limit', t => {
    c.geocode(
        'country',
        { limit_verify: 1, proximity: [-60, -20] },
        (err, res) => {
            t.ifError(err);
            t.equals(res.features[0].place_name, 'country', 'found country');
...
tape('forward country - proximity - single layer - limit', t => {
    c.geocode(
        'country',
        { limit_verify: 1, proximity: [-100, 60] },
        (err, res) => {
            t.ifError(err);
            t.equals(res.features[0].place_name, 'country', 'found country');
            t.equals(res.features[0].id, 'country.1', 'found country.1');
...
tape('forward province - multilayer', t => {
    c.geocode(
        'fakeprov country',
        { proximity: [-100, 60], limit_verify: 1 },
        (err, res) => {
            t.ifError(err);
            t.equals(
                res.features[0].place_name,
...
tape('forward province - multilayer', t => {
    c.geocode(
        'fakeprov country',
        { proximity: [-60, -20], limit_verify: 1 },
        (err, res) => {
            t.ifError(err);
            t.equals(
                res.features[0].place_name,
