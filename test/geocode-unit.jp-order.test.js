    ...
    address: new mem(
        {
            maxzoom: 6,
            geocoder_address: 1,
            geocoder_address_order: 'descending',
            geocoder_format:
                '{country._name}, {region._name}{place._name}{address._name}{address._number}'
        },
        () => {}
    )
};
...
tape('Check order, 岩出市中黒632', t => {
    c.geocode('岩出市中黒632', { limit_verify: 1 }, (err, res) => {
        ...
        t.equal(
            res.features.length,
            1,
            "Descending order doesn't lower relevance"
        );
        t.end();
    });
});
...
tape('Check order, 632 中黒 岩出市', t => {
    c.geocode('632 中黒 岩出市', { limit_verify: 1 }, (err, res) => {
        ...
        t.equal(
            ...
            0.99,
            'Unexpected ascending lowers relevance'
        );
        t.end();
    });
});
