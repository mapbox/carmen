...
test('createReplacer', q => {
    q.deepEqual(
        tokenList,
        tokenClone,
        'createReplacer does not change original value of tokenList'
    );
    q.end();
});
...
test('token replacement', q => {
    q.deepEqual(
        token.replaceToken(tokens, 'fargo street northeast, san francisco'),
        'fargo St NE, sf'
    );
    ...
    q.deepEqual(
        token.enumerateTokenReplacements(
            tokens,
    ...
    q.deepEqual(token.enumerateTokenReplacements(tokens, 'coolstreet'), [
        'coolstreet'
    ]);
    q.deepEqual(token.enumerateTokenReplacements(tokens, 'streetwise'), [
        'streetwise'
    ]);
    q.end();
});
...
test('custom reverse replacement', q => {
    q.deepEqual(
        token.replaceToken(tokensRC, 'st thomas st united states'),
        'saint thomas st united states'
    );
    q.deepEqual(
        token.replaceToken(tokensRC, 'e first st').toLowerCase(),
        'east first street'
    );
    ...
    q.deepEqual(
        ...
        ['st thomas st united states', 'saint thomas st united states']
    );
...
test('named/numbered group replacement', q => {
    ...
    q.deepEqual(token.enumerateTokenReplacements(tokens, 'abc 123 def'), [
        'xyz @@@123@@@ def',
        'xyz 123 def',
        'abc @@@123@@@ def',
        'abc 123 def'
    ]);
    q.deepEqual(token.enumerateTokenReplacements(tokens, 'abc 234 def'), [
        'xyz ###234### def',
        'xyz 234 def',
        'abc ###234### def',
        'abc 234 def'
...
test('make sure word boundaries work right', function(q) {
    q.deepEqual(
        token.replaceToken(tokens, 'Rio de Janeiro'),
        'R de Janeiro',
        'phrase-initial token'
    );
    q.deepEqual(
        token.replaceToken(tokens, 'de rio Janeiro'),
        'de R Janeiro',
        'phrase-medial token'
    );
    q.deepEqual(
        token.replaceToken(tokens, 'de Janeiro Rio'),
        'de Janeiro R',
        'phrase-terminal token'
    );
    q.deepEqual(
        token.replaceToken(tokens, 'de-rio!Janeiro'),
        'de-R!Janeiro',
        'punctuation-separated token'
    );
    q.deepEqual(
        token.replaceToken(tokens, 'deteriorate'),
        'deteriorate',
        "word-medial token (doesn't replace)"
    );
    q.deepEqual(
        token.replaceToken(tokens, 'Rua Oratório'),
        'Rua Oratório',
        "word-terminal token preceded by accented character (doesn't replace)"
    );
    q.end();
});
