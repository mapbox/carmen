...
const conf = {
    country: new mem(
        { maxzoom: 6, geocoder_languages: ['en', 'es'] },
        () => {}
    ),
    ...
    place: new mem(
        {
            maxzoom: 6,
            geocoder_languages: ['en', 'es'],
            geocoder_inherit_score: true
        },
        () => {}
    )
};
...
tape('find nueva york, language=es', t => {
    c.geocode('nueva york usa', { language: 'es' }, (err, res) => {
        ...
        t.equal(
            ...
            0.95,
            "query has penalty applied because 'usa' has no es translation"
        );
        t.end();
    });
});
...
tape('find nueva york, language=ca', t => {
    c.geocode('nueva york', { language: 'ca' }, (err, res) => {
        ...
        t.equal(
            res.features[0].relevance,
            1.0,
            "query has full relevance because 'nueva york' has no ca translation but es falls back"
        );
        t.end();
    });
});
...
tape('find makkah', t => {
    c2.geocode('makkah', {}, (err, res) => {
        t.equal(res.features[0].id, 'place.1');
        t.equal(res.features[0].relevance, 1.0);
        t.end();
    });
});
