...
test('index - streaming interface', t => {
    function getIndex(start, end, fileName) {
        ...
        let inputStream = fs.createReadStream(
            path.resolve(__dirname, './fixtures/' + (fileName || 'docs.jsonl')),
            { encoding: 'utf8' }
        );
    ...
    const indexA = combine(
        getIndex(0, 100),
        getIndex(0, 2, 'docs-normalization.jsonl')
    );
    ...
    const indexB = combine(
        getIndex(100, 200),
        getIndex(2, 4, 'docs-normalization.jsonl')
    );
    t.test('index docs.json', q => {
        carmenB.index(
            indexB,
    ...
    const indexD = combine(
        getIndex(0, 200),
        getIndex(0, 4, 'docs-normalization.jsonl')
    );
    t.test('index docs.json', q => {
        carmenD.index(
            indexD,
    ...
    t.test(
        ...
        q => {
            ...
            for (let i = 1; i <= 202; i++) {
                count += de(
                    memObjectC._shards.feature[i],
                    memObjectD._shards.feature[i],
                    '=='
                )
                    ? 1
                    : 0;
    ...
    ['freq', 'grid'].forEach(type => {
        t.test(
            ...
            q => {
                ...
                let cSet = new Set(
                    carmenC.indexes.country._geocoder[type]
                        .list()
                        .map(stringify)
                );
                let dSet = new Set(
                    carmenD.indexes.country._geocoder[type]
                        .list()
                        .map(stringify)
                );
                let intersection = new Set(
                    Array.from(cSet).filter(x => {
                        return dSet.has(x);
                    })
                );
    ...
    t.test('normalizer comparison', q => {
        var cNormalizations = memObjectC._dictcache.normalizationCache
            .getAll()
            .map(function(row) {
                return [
                    memObjectC._dictcache.lookupCounts(row[0]).text,
                    row[1].map(x => {
                        return memObjectC._dictcache.lookupCounts(x).text;
                    })
                ];
            });
        ...
        var dNormalizations = loaded.normalizationCache
            .getAll()
            .map(function(row) {
                return [
                    loaded.lookupCounts(row[0]).text,
                    row[1].map(x => {
                        return loaded.lookupCounts(x).text;
                    })
                ];
            });
