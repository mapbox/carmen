...
(() => {
    ...
    tape('test address index for relev', t => {
        c.geocode('fake st', { limit_verify: 1 }, (err, res) => {
            t.ifError(err);
            t.equals(
                res.features[0].relevance,
                1.0,
                'token replacement test, fake st'
            );
            t.end();
        });
    });
...
(() => {
    ...
    tape('test address index for relev', t => {
        c.geocode('avenue du 18e régiment', { limit_verify: 1 }, (err, res) => {
            t.ifError(err);
            t.equals(res.features[0].relevance, 1.0, 'avenue du 18e');
            t.end();
        });
    });
    tape('test address index for relev', t => {
        c.geocode(
            'avenue du dix-huitième régiment',
            { limit_verify: 1 },
            (err, res) => {
                t.ifError(err);
                t.equals(
    ...
    tape('test token replacement', t => {
        c.geocode('qabc', { limit_verify: 1 }, (err, res) => {
            ...
            t.equals(
                res.features[0].relevance,
                1.0,
                'token regex numbered group test, qabc => qcba'
            );
            t.end();
        });
    });
...
(() => {
    ...
    tape('test address index autocomplete + tokens (auto)', t => {
        c.geocode(
            'main road',
            { limit_verify: 1, autocomplete: false },
            (err, res) => {
                t.ifError(err);
                t.equals(
                    res.features[0].place_name,
    ...
    tape('test token replacement', t => {
        c.geocode('Talstrasse', { limit_verify: 1 }, (err, res) => {
            ...
            t.equals(
                res.features[0].relevance,
                1.0,
                'token replacement for str -> strasse'
            );
            t.end();
        });
    });
    ...
    tape('test token replacement', t => {
        c.geocode('Talstrassesomthing', { limit_verify: 1 }, (err, res) => {
            ...
            t.deepEquals(
                res.features,
                [],
                'strasse token is not replaced when present in between a word'
            );
            t.end();
        });
    });
...
(() => {
    ...
    ['burbarg', 'bürbarg', 'buerbarg'].forEach(query => {
