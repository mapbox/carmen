...
test('ops#toFeature', t => {
    ...
    feat._relevance = 1;
    t.deepEqual(ops.toFeature(feat, '{address._name} {address._number}'), {
        address: 9,
        center: [-99.392855, 63.004759],
        geometry: { coordinates: [-99.392855, 63.004759], type: 'Point' },
        id: 'address.1833980151',
        place_name: 'Fake Street 9',
        place_type: ['address'],
        properties: {},
        relevance: 1,
        text: 'Fake Street',
    ...
    t.deepEqual(
        ops.toFeature(
            ...
            '{address._number} {address._name}'
        ).place_name,
        '9 Fake Street',
        'Address number & name exist'
    );
    ...
    t.deepEqual(
        ops.toFeature(
            ...
            '{address._number} {address._name}, {place._name}'
        ).place_name,
        '9 Fake Street, Andor',
        'Address & Place'
    );
    ...
    t.deepEqual(
        ops.toFeature(
            ...
            '{address._number} {address._name}, {place.name}'
        ).place_name,
        '9 Fake Street',
        'Address & no Place'
    );
    ...
    t.deepEqual(
        ops.toFeature(
            ...
            '{address._number} {address.name}, {place._name}'
        ).place_name,
        '9, Andor',
        'No Address street & Place'
    );
    ...
    t.deepEqual(
        ops.toFeature(
            fullStack,
            '{address._number} {address._name}, {place._name}, {region._name} {postcode._name}'
        ).place_name,
        'Fake Street, Caemlyn, Andor 1234',
        'Full stack'
    );
    t.deepEqual(
        ops.toFeature(
            fullStack,
            '{address._number} {address._name}, {place.name}, {region._name} {postcode._name}'
        ).place_name,
        'Fake Street, Andor 1234',
        'Full stack'
    );
    t.equals(
        ops.toFeature(fullStack).context.pop().short_code,
        'ca',
        'short_code property made it into context array'
    );
