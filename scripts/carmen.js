...
if (argv.help) {
    ...
    console.log(
        '  --limit="{limit}"       Customize the number of results returned'
    );
    console.log('  --proximity="lat,lng"   Favour results by proximity');
    console.log(
        '  --types="{type},..."    Only return results of a given type'
    );
    console.log(
        '  --stacks="{stack},..."  Only return results of a given stack'
    );
    console.log('  --geojson               Return a geojson object');
    console.log(
        '  --language={ISO code}   Return responses in specified language (if available in index)'
    );
    console.log('  --stats                 Generate Stats on the query');
    console.log('  --debug="feat id"       Follows a feature through geocode"');
    console.log(
        '  --reverseMode="{mode}"  Sort features in reverse queries by one of `score` or `distance`'
    );
...
if (argv.reverseMode) {
    if (argv.reverseMode !== 'score' && argv.reverseMode !== 'distance')
        throw new Error('reverseMode must be one of `score` or `distance`');
}
        ...
        if (argv.debug && data.debug) {
            ...
            Object.keys(data.debug.phrasematch).forEach(function(idx) {
                console.log(
                    '  ',
                    idx,
                    JSON.stringify(data.debug.phrasematch[idx])
                );
            });
            console.log();
            ...
            console.log('------------');
            console.log(
                'spatialmatch position:',
                data.debug.spatialmatch_position
            );
            ...
            console.log('-----------');
            console.log(
                'verifymatch position:',
                data.debug.verifymatch_position
            );
            console.log(JSON.stringify(data.debug.verifymatch, null, 2));
            console.log();
        } else if (!data.debug)
            console.log(
                'No debug information collected (is this a reverse query?)'
            );
