...
process.on('message', function(data) {
    ...
    q.awaitAll(function() {
        var carmens = [
            new Carmen(inputConfs[0]),
            new Carmen(inputConfs[1]),
            new Carmen(outputConf)
        ];
        var openq = queue();
        carmens.forEach(function(carmen) {
            openq.defer(function(callback) {
        ...
        openq.awaitAll(function() {
            console.log(
                '# starting merge of ' +
                    inputConfs[0].from._original.filename +
                    ' and ' +
                    inputConfs[1].from._original.filename +
                    ' into ' +
                    outputConf.to._original.filename
            );
            carmens[2].merge(
                inputConfs[0].from,
                inputConfs[1].from,
