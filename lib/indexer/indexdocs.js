...
function indexdocs(docs, source, options, callback) {
    ...
    if (options.zoom < 0)
        return callback(
            new Error(
                'zoom must be greater than 0 --- zoom was ' + options.zoom
            )
        );
    if (options.zoom > 14)
        return callback(
            new Error('zoom must be less than 15 --- zoom was ' + options.zoom)
        );
    ...
    var settings = {
        source: source,
        zoom: options.zoom,
        geocoder_tokens: options.geocoder_tokens,
    ...
    try {
        freq = generateFrequency(
            docs,
            source.token_replacer,
            options.tokens,
            parseInt(source.maxscore),
            source.lang.has_languages
        );
    } catch (err) {
        return callback(err);
    }
    ...
    for (var i = 0; i < ids.length; i++) {
        ...
        if (id === '__MAX__') {
            freq[id][0] =
                (source._geocoder.freq.get(id) || [0, 0])[0] || freq[id][0];
        }
    ...
    if (!source.lang.fallback_matrix) {
        var matrix = cl.fallbackMatrix(
            source.lang.languages.filter(function(l) {
                return l != 'default';
            })
        );
...
function parseDocs(docs, settings, full) {
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        if (docs[i].properties['carmen:addressnumber']) {
            for (
                c_it = 0;
                c_it < docs[i].properties['carmen:addressnumber'].length;
                c_it++
            ) {
                ...
                for (
                    addr_it = 0;
                    addr_it <
                    docs[i].properties['carmen:addressnumber'][c_it].length;
                    addr_it++
                ) {
                    feat = point(
                        docs[i].geometry.geometries[c_it].coordinates[addr_it],
                        feature.storableProperties(docs[i].properties, 'vector')
                    );
                    feat.id = docs[i].id;
        ...
        if (docs[i].properties['carmen:rangetype']) {
            for (c_it = 0; c_it < docs[i].geometry.geometries.length; c_it++) {
                if (
                    docs[i].geometry.geometries[c_it].type !== 'MultiLineString'
                )
                    continue;
                ...
                for (
                    addr_it = 0;
                    addr_it <
                    docs[i].geometry.geometries[c_it].coordinates.length;
                    addr_it++
                ) {
                    feat = linestring(
                        docs[i].geometry.geometries[c_it].coordinates[addr_it],
        ...
        if (
            !docs[i].properties['carmen:addressnumber'] &&
            !docs[i].properties['carmen:rangetype']
        ) {
            full.vectors.push({
                id: docs[i].id,
                type: 'Feature',
                geometry: docs[i].geometry,
                properties: feature.storableProperties(
                    docs[i].properties,
                    'vector'
                )
            });
...
function runChecks(doc) {
    ...
    if (!doc.id) {
        ...
        for (var err_it = 0; err_it < geojsonErr.length; err_it++) {
            if (geojsonErr[err_it].level == 'message') {
                ...
                !geojsonErr[err_it].message.match(
        ...
        doc.properties['carmen:geocoder_stack'] &&
    ...
    if (
        ...
        doc.geometry.type === 'MultiPolygon'
    ) {
        ...
        if (vertices > 50000) {
            throw Error(
                'Polygons may not have more than 50k vertices. Simplify your polygons, or split the polygon into multiple parts on id:' +
                    doc.id
            );
        }
    }
...
function standardize(doc, zoom) {
    ...
    var tiles = [];
    if (
        doc.geometry.type === 'GeometryCollection' &&
        ...
        for (
            var feat_it = 0;
            feat_it < doc.geometry.geometries.length;
            feat_it++
        ) {
            tiles = tiles.concat(
                cover.tiles(doc.geometry.geometries[feat_it], {
                    min_zoom: zoom,
                    max_zoom: zoom
                })
            );
        ...
        tiles.forEach(function(tile) {
            doc.properties['carmen:zxy'].push(
    ...
    if (
        !doc.properties['carmen:center'] ||
        !verifyCenter(doc.properties['carmen:center'], tiles)
    ) {
        console.warn(
            'carmen:center did not fall within the provided geometry for %s (%s). Calculating new point on surface.',
            doc.id,
            doc.properties['carmen:text']
        );
        doc.properties['carmen:center'] = centroid(
            doc.geometry
        ).geometry.coordinates;
        if (!verifyCenter(doc.properties['carmen:center'], tiles)) {
            throw Error(
                'Invalid carmen:center provided, and unable to calculate corrected centroid. Verify validity of doc.geometry for doc id:' +
                    doc.id
            );
        } else {
            console.warn(
    ...
    if (!doc.properties['carmen:zxy']) {
        throw Error(
            "doc.properties['carmen:zxy'] undefined, failed indexing, doc id:" +
                doc.id
        );
    }
    ...
    if (
        doc.properties['carmen:zxy'] &&
        doc.properties['carmen:zxy'].length > 10000
    ) {
        console.warn(
            'carmen:zxy exceeded 10000, truncating to 10000 (doc id: %s, text: %s)',
            doc.id,
            doc.properties['carmen:text']
        );
        ...
        sortedCovers.sort(function(a, b) {
            ...
            b = b.split('/');
            var aDist = Math.sqrt(
                Math.pow(centerCover[1] - parseInt(a[1], 10), 2) +
                    Math.pow(centerCover[2] - parseInt(a[2], 10), 2)
            );
            var bDist = Math.sqrt(
                Math.pow(centerCover[1] - parseInt(b[1], 10), 2) +
                    Math.pow(centerCover[2] - parseInt(b[2], 10), 2)
            );
            return aDist - bDist;
        });
...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    var scaledScore =
        termops.encode3BitLogScale(doc.properties['carmen:score'], maxScore) ||
        0;

    var texts = termops.getIndexableText(
        token_replacer,
        globalTokens,
        doc,
        source ? source.use_normalization_cache : false
    );
    ...
    for (var x = 0; x < texts.length; x++) {
        ...
        for (y = 0; y < phrases.length; y++) {
            ...
            if (phrases[y].relev > phraseObj.phrase.relev)
                phraseObj.phrase.relev = phrases[y].relev;

            texts[x].languages.forEach(function(l) {
                phraseObj.languages.add(l);
            });
        ...
        if (texts[x].variants) {
            for (y = 0; y < texts[x].variants.length; y++) {
                variants.set(
                    termops.encodableText(texts[x].variants[y].join(' ')),
                    termops.encodableText(texts[x].tokens.join(' '))
                );
            }
        }
    }
    ...
    for (var phraseValue of allPhrases.values()) {
        for (var phraseLang of phraseValue.languages) {
            if (!phrasesByLanguage.has(phraseLang))
                phrasesByLanguage.set(phraseLang, []);
    ...
    for (var phraseItem of allPhrases) {
        ...
        var langList = Array.from(languages)
            .sort()
            .join(',');
...
function generateFrequency(docs, replacer, globalTokens, maxScore) {
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        freq['__MAX__'][0] =
            ...
            Math.max(
                freq['__MAX__'][0],
                docs[i].properties['carmen:score'] || 0
            );
