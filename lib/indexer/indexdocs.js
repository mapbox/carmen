...
function parseDocs(docs, settings, full) {
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        //Collapse FeatureCollections into single features
        // docs are typically of type Feature. FeatureCollections
        // should currently only be used for consolidating pt/itp
        // address sources
        if (docs[i].type === 'FeatureCollection') {
            var cDoc = { type: 'Feature', properties: {}, geometry: { type: 'MultiLineString', coordinates: [] } };

            for (var feat_it = 0; feat_it < docs[i].features.length; feat_it++) {
                var feat = docs[i].features[feat_it];
            }
        }
...
function runChecks(doc, zoom) {
    ...
    } else if (doc.type !== 'Feature' || doc.type !== 'FeatureCollection') {
