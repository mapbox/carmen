...
function standardize(doc, zoom) {
    ...
    if (doc.properties['carmen:addressnumber'] && doc.geometry.type === 'GeometryCollection') {
        const outlierBounds = getNonOutlierBounds(doc.properties['carmen:addressnumber']);
        // Check if outliers are present
        if (outlierBounds[4] > 0) {
                        ...
                        if (doc.properties[p][i] != null) {
                            ...
                            for (let j = 0; j < doc.properties[p][i].length; j++) {
                                if (doc.properties[p][i][j] != null) {
                                    ...
                                    if (doc.properties[p][i][j] < outlierBounds[0]) {
                                        doc.properties[p][i][j] = outlierBounds[1];
                                    }
                                    if (doc.properties[p][i][j] > outlierBounds[2]) {
                                        doc.properties[p][i][j] = outlierBounds[3];
                                    }
                                }
                            }
                        }
 ...
 * @return {Array} [lowerLimit, lowerBound, upperLimit, upperBound, outlierCont]
 */
function getNonOutlierBounds(addressnumber) {
    ...
    let upperLimit, lowerLimit, lowerPercentile, upperPercentile;
    ...
    for (let i = 0; i < addressnumber.length; i++) {
        if (addressnumber[i] != null) {
            ...
            lowerLimit = lowerPercentile - (deltaPerPercentile * (targetPercentile + errorPercentile));
            upperLimit = upperPercentile + (deltaPerPercentile * (targetPercentile + errorPercentile));

            const lowerBound = sortedArray.find((value) => value >= lowerLimit);
            const upperBound = sortedArray.slice().reverse().find((value) => value <= upperLimit);
            const outlierCont = sortedArray.filter((value) => (value < lowerLimit || value > upperLimit)).length;
            return [lowerLimit, lowerBound, upperLimit, upperBound, outlierCont];
        }
    }
