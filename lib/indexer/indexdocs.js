...
function loadDoc(patch, doc, freq, zoom, token_replacer) {
    ...
    if (doc.type === 'FeatureCollection') {
        ...
        for (var feat_it = 0; feat_it < doc.features.length; feat_it++) {
            ...
            tiles = tiles.concat(cover.tiles(feat.geometry, {min_zoom: zoom, max_zoom: zoom}));
        ...
        tiles = tiles.concat(cover.tiles(doc.geometry, {min_zoom: zoom, max_zoom: zoom}));
    ...
    if (!doc.properties['carmen:zxy']) {
        if (tiles.length === 0) tiles = tiles.concat(cover.tiles(feat.geometry, {min_zoom: zoom, max_zoom: zoom}));
