        ...
        q.awaitAll(function(err) {
            ...
            to.stopWriting(function(err) {
                ...
                copyQ.await(function() {
                    to._geocoder.grid = new cxxcache.RocksDBCache(
                        to._geocoder.grid.id,
                        rocksDirs.grid
                    );
                    to._geocoder.freq = new cxxcache.RocksDBCache(
                        to._geocoder.freq.id,
                        rocksDirs.freq
                    );
                    callback();
                });
...
function geocoderPutStream(to, type) {
    ...
    putStream._transform = function(row, enc, callback) {
        if (putStream.pending > 1000) {
            return setImmediate(
                putStream._transform.bind(putStream),
                row,
                enc,
                callback
            );
