    ...
    for (var i = 0; i < loaded.length; i++) {
        ...
        for (var l = feature.properties['carmen:types'].length - 1; l >= 0; l--) {
            ...
            if (memo[type] === undefined) {
                ...
                if (reverseMode === 'distance' && !feature.properties['carmen:reverseMode'] && feature.properties['carmen:vtquerydist'] >= memo[type].properties['carmen:vtquerydist']) continue;
                ...
                // TODO: this probably should be tightned up
                if (reverseMode === 'distance' && !feature.properties['carmen:reverseMode']) {
...
function contextVector(source, lon, lat, full, matched, language, scoreFilter, reverseMode, callback) {
    ...
    function query(err, vt) {
        ...
        vt.query(lon, lat, {
            ...
            if (reverseMode === 'score' && full && source.type === 'poi') {
                for (var j = 0; j < results.length; j++) {
                    resultMap[results[j].id()] = results[j].attributes();
                }

                results = sortByDistScore(results, resultMap);
            }
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (results[i].distscore) {
                    feat = results[i].attributes();
                    break;
                }
...
function fullFeature(source, feat, query, callback) {
    feature.getFeatureById(source, feat.id, function(err, loaded) {
        ...
        loaded.properties['carmen:stack'] = source.stack;
        loaded.properties['carmen:conflict'] = source.name !== source.type ? source.name : undefined;
        loaded.properties['carmen:reverseMode'] = source.type === 'poi' ? true : false;
...
function sortByDistScore(results, resultMap) {
    return results.sort(function (a, b) {
        var attr_a = resultMap[a.id()];
        var attr_b = resultMap[b.id()];

        var a_dist = a.distance;
        var b_dist = b.distance;

        var a_score = attr_a['carmen:score'];
        var b_score = attr_b['carmen:score'];

        var a_distscore = proximity.distscore(a_dist, a_score);
        var b_distscore = proximity.distscore(b_dist, b_score);
        a.distscore = a_distscore;
        b.distscore = b_distscore;
        return  b_distscore - a_distscore;
    });
}