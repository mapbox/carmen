...
module.exports = function(geocoder, position, options, callback) {
    ...
    var maxidx =
        typeof options.maxidx === 'number' ? options.maxidx : index_ids.length;
    ...
    for (
        ...
        if (
            ...
            if (
                options.full &&
                subtypeLookup[source.type] &&
                source.scoreranges[subtypeLookup[source.type]]
            )
                scoreRange = [
                    source.scoreranges[subtypeLookup[source.type]][0] *
                        source.maxscore,
                    source.scoreranges[subtypeLookup[source.type]][1] *
                        source.maxscore
                ];
            ...
            if (
                ...
                source.type === indexes[index_ids[index_ids.length - 1]].type
            ) {
...
function getSubtypeLookup(types) {
    ...
    for (var type_i = 0; type_i < types.length; type_i++) {
        ...
        else subtypeLookup[splitType[0]] = true;
    }
...
function stackFeatures(geocoder, loaded, options) {
    ...
    for (var i = 0; i < loaded.length; i++) {
        ...
        for (
            var l = feature.properties['carmen:types'].length - 1;
            l >= 0;
            l--
        ) {
            ...
            if (memo[type] === undefined) {
                ...
                feature.properties['carmen:extid'] =
                    type +
                    '.' +
                    feature.properties['carmen:extid'].split('.').pop();
                break;
            } else if (
                memo[type] &&
                feature.properties['carmen:geomtype'] !== 3
            ) {
                if (reverseMode === 'score') {
                    // don't replace a higher-scored feature with a lower-scored one
                    if (
                        !feature.properties['carmen:score'] &&
                        memo[type].properties['carmen:score']
                    )
                        continue;
                    if (
                        feature.properties['carmen:score'] &&
                ...
                if (
                    feature.properties['carmen:vtquerydist'] >=
                    memo[type].properties['carmen:vtquerydist']
                )
                    continue;
    ...
    for (
        ...
        if (
            ...
            if (
                typeSplit.length === 2 &&
                source.scoreranges &&
                source.scoreranges[typeSplit[1]]
            ) {
                scoreFilter = [
...
var getTile = Locking(
    function(options, unlock) {
        ...
        source.getTile(z, x, y, function(err, zpbf) {
            ...
            if (!compression)
                return unlock(
                    new Error('Could not detect compression of vector tile')
                );
...
function nearestPoints(source, lon, lat, scoreFilter, callback) {
    ...
    function afterQuery(err, results) {
        ...
        for (var results_it = 0; results_it < results.length; results_it++) {
            ...
            if (
                scoreFilter &&
                (score <= scoreFilter[0] || score > scoreFilter[1])
            )
                continue;
            ...
            function(err, results) {
                ...
                if (
                    ...
                    mapped = results.map(function(el, i) {
                        return {
                            index: i,
                            distscore: proximity.distscore(
                                el.distance,
                                el.attributes()['carmen:score']
                            )
                        };
                    });
                ...
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (matched._exclusive) {
                        if (matched[tmpid]) {
                            attr['carmen:vtquerydist'] = results[i].distance;
                            attr['carmen:geomtype'] = results[i]
                                .geometry()
                                .type();
                            feat = attr;
                            break;
                    ...
                    var score =
                        attr['carmen:score'] ||
                        (attr.properties && attr.properties['carmen:score']);
                    if (score === undefined) score = 0;
...
function lightFeature(source, feat, callback) {
    ...
    loaded.properties['carmen:tmpid'] =
        source.idx * mp25 + termops.feature(feat.id);
    loaded.properties['carmen:index'] = source.id;
    loaded.properties['carmen:stack'] = source.stack;
    loaded.properties['carmen:conflict'] =
        source.name !== source.type ? source.name : undefined;

    // Convert types, center attributes from serialized strings to real values.
    if (typeof loaded.properties['carmen:types'] === 'string') {
        loaded.properties['carmen:types'] =
            loaded.properties['carmen:types'][0] === '['
    ...
    if (typeof loaded.properties['carmen:center'] === 'string') {
        loaded.properties['carmen:center'] =
            loaded.properties['carmen:center'][0] === '['
                ? JSON.parse(loaded.properties['carmen:center'])
                : loaded.properties['carmen:center'].split(',');
        loaded.properties['carmen:center'][0] = parseFloat(
            loaded.properties['carmen:center'][0]
        );
        loaded.properties['carmen:center'][1] = parseFloat(
            loaded.properties['carmen:center'][1]
        );
    }
...
function fullFeature(source, feat, query, callback) {
    feature.getFeatureById(source, feat.id, function(err, loaded) {
        ...
        if (
            ...
            loaded.properties['carmen:addressnumber']
        ) {
        ...
        if (addrpt && addritp) {
            ...
            if (
                distance(qFeat, addritp) < distance(qFeat, addrpt) &&
                distance(addrpt, addritp, 'kilometers') > 0.2
            ) {
                loaded = addritp;
            } else {
                loaded = addrpt;
