...
// @param {String} reverse: boolean of whether to do a reverse geocode
//                          A reverse geocode loads all features on tile (slower)
// @param {Function} callback
module.exports = function(geocoder, lon, lat, maxidx, reverse, callback) {
    var context = [];
    ...
    types.forEach(function(type) {
        ...
        if (lat >= bounds[1] && lat <= bounds[3] && lon >= bounds[0] && lon <= bounds[2])
            q.defer(method, source, lon, lat, reverse);
    });
...
function contextVector(source, lon, lat, full, callback) {
    ...
    function query(err, vt) {
        ...
        //Increase tolerance for reverse geocodes
        var tol = full ? 200 : 50;
