...
function rebalance(query, stack, flattenedPhrasematches) {
    ...
    for (let k = 0; k < stackLength; k++) {
        const entry = JSON.parse(JSON.stringify(stack.entries[k]));
        ...
        if (missingQueryTokens > 0) {
            ...
            entry.grid_entry.relev = roundTo(((entry.grid_entry.relev + stackWeight * entryDiscountFactor) / 2) + entryTokenBonus * entryTokenDifference, 8);
        }
