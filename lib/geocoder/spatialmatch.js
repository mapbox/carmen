...
const constant_values = `${constants.STACKABLE_LIMIT},${constants.SPATIALMATCH_STACK_LIMIT},${constants.VERIFYMATCH_STACK_LIMIT}`;
...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    let stacks;
    console.log(
        `--- SpatialMatch Profile ---\nSTACKABLE_LIMIT,SPATIALMATCH_STACK_LIMIT,VERIFYMATCH_STACK_LIMIT,phase,time`);
    ...
    if (phrasematchResults.length) {
        ...
        console.log(`${constant_values},sortByZoomIdx,${diff}`);
        arch_stacks.sort(sortByRelevLengthIdx);
        diff = hrtime_format(process.hrtime(spatialmatchStart));
        console.log(`${constant_values},sortByRelevLengthIdx,${diff}`);
    ...
    function coalesceStack(stack, callback) {
        ...
        coalesce(stack, coalesceOpts, (err, cacheSpatialmatches) => {
            ...
            diff = hrtime_format(process.hrtime(spatialmatchStart));
            console.log(`${constant_values},coalesceStack,${diff}`);
    ...
    function coalesceFinalize(err, results) {
        ...
        diff = hrtime_format(process.hrtime(spatialmatchStart));
        console.log(`${constant_values},coalesceFinalize,${diff}`);

        console.log(`--- End SpatialMatch Profile ---\n--------------------------------`);
...
function collapseToArchetypes(phrasematchResults) {
    ...
    diff = hrtime_format(process.hrtime(spatialmatchStart));
    console.log(`${constant_values},collapseToArchetypes,${diff}`);
    return outResults;
}
...
function expandFromArchetypesInner(stack, maxOut, matchIdx, soFar, out) {
    if (matchIdx === stack.length - 1) {
        ...
        diff = hrtime_format(process.hrtime(spatialmatchStart));
        console.log(`${constant_values},expandFromArchetypesInner,${diff}`);
        return false;
    } else {
        for (const exemplar of stack[matchIdx].exemplars) {
            const outStack = soFar.slice();
...
function allowed(stacks, options) {
    ...
    diff = hrtime_format(process.hrtime(spatialmatchStart));
    console.log(`${constant_values},allowed,${diff}`);
    return filtered;
}
...
function stackable(phrasematchResults, limit, memo, idx, mask, nmask, stack, relev, adjRelev) {
    ...
    if (idx === 0) {
        ...
        diff = hrtime_format(process.hrtime(spatialmatchStart));
        console.log(`${constant_values},stackable,${diff}`);
        return stacks;
    }
}
...
function rebalance(query, stack) {
    ...
    diff = hrtime_format(process.hrtime(spatialmatchStart));
    console.log(`${constant_values},rebalance,${diff}`);
    return stackClone;
}
