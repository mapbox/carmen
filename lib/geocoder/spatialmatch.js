...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    function coalesceStacks() {
        coalesce_all_start = process.hrtime();
        const q = queue();
    ...
    function coalesceFinalize(err, results) {
        ...
        const coalesce_all_duration = process.hrtime(coalesce_all_start);
        console.log(`
        ---
            STACKABLE_LIMIT: ${constants.STACKABLE_LIMIT},
            SPATIALMATCH_STACK_LIMIT: ${constants.SPATIALMATCH_STACK_LIMIT},
            VERIFYMATCH_STACK_LIMIT: ${constants.VERIFYMATCH_STACK_LIMIT},
            coalesce_all_duration: ${coalesce_all_duration[0] * NS_PER_SEC + coalesce_all_duration[1]}ns
        ---
        `)
        let combined = [];
