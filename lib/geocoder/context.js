...
function processVtQueryResults(results, reverseMode, full, source, matched, scoreFilter, lon, lat, routing, callback) {
...
function contextVector(source, lon, lat, full, matched, language, scoreFilter, reverseMode, routing, callback) {
    ...
    function query(err, vt) {
        ...
        vtquery([vt], [lon, lat], {
            ...
            if (Object.entries(matched).length === 0 && reverseMode === 'distance') {
                let atLeastOneGoodFeature = false;
                for (let i = 0; i < results.length; i++) {
                    const attr = results[i].properties;
                    const score = attr['carmen:score'] || (attr.properties && attr.properties['carmen:score']);
                    if (score >  0) {
                        atLeastOneGoodFeature = true;
                    }
                }
                if (!atLeastOneGoodFeature) {
                    vtquery([vt], [lon, lat], {
                        radius: 1000,
                        layer: source.geocoder_layer,
                        limit: 100,
                        ...
                        return processVtQueryResults(results, reverseMode, full, source, matched, scoreFilter, lon, lat, routing, callback);
                    });
                } else {
                    return processVtQueryResults(results, reverseMode, full, source, matched, scoreFilter, lon, lat, routing, callback);
                }
