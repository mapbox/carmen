...
function contextVector(source, lon, lat, full, matched, language, scoreFilter, reverseMode, routing, callback) {
    ...
    function query(err, vt) {
        ...
        vtquery([vt], [lon, lat], {
            ...
            // Check if there is at least one non-negative scoring feature
            let allNegativeScoreFeatures = true;
            for (let i = 0; i < results.length; i++) {
                const attr = results[i].properties;
                const score = attr['carmen:score'] || (attr.properties && attr.properties['carmen:score']);
                if (score >= 0) {
                    allNegativeScoreFeatures = false;
                }
            }


            // In distance based reverse geocode not against a specific feature, requery if all results are negative scoring
            if (Object.entries(matched).length === 0 && reverseMode === 'distance' && allNegativeScoreFeatures) {
                vtquery([vt], [lon, lat], {
                    radius: 1000,
                    layer: source.geocoder_layer,
                    limit: 100,
