    ...
    for (let k = 0; k < result.length; k++) {
        ...
        if (options.proximity) {
            feat.properties['carmen:scoredist'] = proximity.scoredist(
                ...
                geocoder.minScore,
                geocoder.maxScore[feat.properties['carmen:types'].slice(-1)[0]],
...
function verifyContext(context, peers, strict, loose, indexes, options, geocoder) {
    ...
    if (squishy > 0) {
        ...
        const featMaxScore = geocoder.maxScore[feat.properties['carmen:types'].slice(-1)[0]];
        ...
        if (options.proximity) {
            feat.properties['carmen:scoredist'] = proximity.scoredist(
                Math.min(feat.properties['carmen:score'] + squishy, featMaxScore),
                geocoder.minScore,
                featMaxScore,
                feat.properties['carmen:distance'],
                feat.properties['carmen:zoom']
