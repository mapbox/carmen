...
WriteCache.prototype.setNormalization = function(nonstandard, standard) {
    ...
    if (!this.normalizationMap.has(nonstandard))
        this.normalizationMap.set(nonstandard, new Set());
...
WriteCache.prototype.dumpWithNormalizations = function(normalizationCachePath) {
    ...
    for (var i = 0; i < prefixes.length; i++) {
        ...
        for (var j = 0; j < phrases.length; j++) {
            ...
            if (
                this.normalizationMap.has(phraseString) ||
                this.normalizationTargets.has(phraseString)
            ) {
                normalizationIndexes.set(phraseString, idx);
                if (this.data.get(prefixes[i]).get(phraseString))
                    mapsToSelf.add(phraseString);
            }
    ...
    for (let [key, values] of this.normalizationMap) {
        ...
        normData.push([
            ...
            Array.from(values)
                .map(v => {
                    return normalizationIndexes.get(v);
                })
                .sort()
        ]);
    }
...
ReadCache.prototype.hasPhraseOrNormalizations = function(text, ender) {
    ...
    if (match.found) {
        if (this.normalizationCache) {
            if (ender) {
                norms = norms.concat(
                    this.normalizationCache.getPrefixRange(
                        match.index,
                        match.suffixCount
                    )
                );
            ...
            let _this = this;
            match.normalizations = norms.map(function(idx) {
                return _this.lookupCounts(idx).text;
            });
        }
        return match;
    } else {
...
ReadCache.prototype.loadNormalizationCache = function(normalizationCachePath) {
    this.normalizationCache = new NormalizationCache(
        normalizationCachePath,
        true
    );
};
...
WriteCache.prototype.hasPhrase = function(text, ender) {
    if (DEBUG)
        console.warn(
            'calling hasPhrase on a DAWG WriteCache is very inefficient'
        );
...
WriteCache.prototype.hasPhraseOrNormalizations = function(text, ender) {
    if (DEBUG)
        console.warn(
            'calling hasPhraseOrNormalizations on a DAWG WriteCache is very inefficient'
        );
...
WriteCache.prototype.iterator = function(string) {
    if (DEBUG)
        console.warn(
            'calling iterator on a DAWG WriteCache is very inefficient'
        );
