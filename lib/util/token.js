...
module.exports.createReplacer = function(tokens, inverseOpts) {
    ...
    for (var token in tokens) {
        ...
        for (var u = 0; u < 2; u++) {
            ...
            if (
                entry.named &&
                parsedFrom.xregexp.captureNames.some(function(captureName) {
                    return captureName === null;
                })
            )
                throw new Error(
                    "Cannot process '%s'; must use either named or numbered capture groups, not both",
                    from
                );
            ...
            if (entry.named) {
                ...
                if (typeof to == 'string' && !replacementOpts.skipBoundaries) {
                    to = to.replace(/\$(\d+)/g, function(str, index) {
...
module.exports.replaceToken = function(tokens, query) {
    ...
    for (let token of tokens) {
        abbr = abbr.trim();
        if (token.named) abbr = XRegExp.replace(abbr, token.from, token.to);
        else abbr = abbr.replace(token.from, token.to);
    }
...
module.exports.enumerateTokenReplacements = function(tokens, query) {
    ...
    for (let token of tokens) {
        ...
        for (var i = 0; i < Math.min(out.length, 64); i++) {
            ...
            var occurrenceCount = token.named
                ? 1
                : (out[i].match(token.from) || []).length;
