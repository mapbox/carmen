...
module.exports.crossDateline = crossDateline;
/**
...
/**
* crossDateline - Return a smaller bbox when a feature straddles the dateline
...
function crossDateline(geom, boundingBox) {
    const eastHemiBBox = [-180,-90,0,90];
    const westHemiBBox = [0,-90,180,90];
    ...
    const amBBox = [bboxWest[0],bboxTotal[1],bboxEast[2],bboxTotal[3]];
    const pmBBox = [bboxEast[0],bboxTotal[1],bboxWest[2],bboxTotal[3]];

    const pmBBoxWidth = (bboxWest[2]) + Math.abs(bboxEast[0]);
    const amBBoxWidth = (180 - bboxWest[0]) + (180 - Math.abs(bboxEast[2]));
