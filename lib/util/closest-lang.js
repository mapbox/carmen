...
var getLanguage = function(str) {
    ...
    if (str.length > 1) {
        ...
        str.forEach(function(d, i) {
            var obj =
                munLanguages[i == 0 ? languageOnlyRef[d] : nonLanguageRef[d]];
            if (!obj && d.search(/[A-Z]/) != -1) {
                var ld = d.toLowerCase();
                obj =
                    munLanguages[
                        i == 0 ? languageOnlyRef[ld] : nonLanguageRef[ld]
                    ];
            }
...
var getScriptComponent = function(mLang) {
    ...
    var scripts = mLang.filter(function(item) {
...
var getLanguageComponent = function(mLang) {
    var languages = mLang.filter(function(item) {
...
var getLanguageArray = function(lang) {
    ...
    return {
        code: lang,
        subtags: Array.isArray(munLang) ? munLang : [munLang]
    };
};
...
var getScoredCandidates = function(target, candidateList) {
    ...
    var munCandidates = candidateList.map(function(item) {
        return getLanguageArray(item);
    });
    ...
    munCandidates.forEach(function(candidate) {
        ...
        if (candidateLanguage && languageBonuses[candidateLanguage])
            candidate.score += languageBonuses[candidateLanguage];
        if (candidateScript && scriptBonuses[candidateScript])
            candidate.score += scriptBonuses[candidateScript];
    ...
    munCandidates.sort(function(a, b) {
...
var closestLangLabel = function(target, candidates, prefix, languageMode) {
    ...
    if (prefix) {
        ...
        regexCandidates = Object.keys(candidates)
            ...
            .filter(function(item) {
                return regexPrefix.exec(item);
            })
            .map(function(item) {
                return item.replace(regexPrefix, '');
            });
    } else {
        regexCandidates = Object.keys(candidates).filter(function(item) {
            return !!candidates[item];
        });
    }
    ...
    for (i = 0; i < regexCandidates.length; i++) {
        if (regexCandidates[i].toLowerCase() == target.toLowerCase())
    ...
    if (fb)
        for (fb_i = 0; fb_i < fb.length; fb_i++)
            if (candidates[prefix + fb[fb_i]]) return fb[fb_i];
    ...
    if (
        winner.score < 50 ||
        (languageMode === 'strict' && digraphic.indexOf(winner.code) > -1)
    )
        return;
    return winner.code;
};
...
var fallbackRanking = function(target, candidateList) {
    ...
    var fb = fallbackIndexer[target.toLowerCase()],
        fb_i;
    if (fb)
        for (fb_i = 0; fb_i < fb.length; fb_i++) {
    ...
    for (var i = 0; i < candidateList.length; i++) {
        if (
            candidateList[i].toLowerCase() == primary &&
            candidateList[i].toLowerCase() != target
        ) {
...
function getText(language, properties) {
    if (!properties['carmen:text'])
        throw new Error('Feature has no carmen:text');
    if (!language)
        return { text: properties['carmen:text'].split(',')[0].trim() };
    var languageLabel = closestLang.closestLangLabel(
        language,
        properties,
        'carmen:text_'
    );
    var languageText = languageLabel
        ? properties['carmen:text_' + languageLabel]
        : false;
    var text = {
        text: (languageText || properties['carmen:text'] || '')
            .split(',')[0]
            .trim()
    };
