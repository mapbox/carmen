...
function sourceAllowed(source, options) {
    var allowed = true;
    if (options.stacks)
        allowed = allowed && sourceMatchesStacks(source, options);
...
function sourceMatchesTypes(source, options) {
    for (var t = 0; t < source.types.length; t++) {
        ...
        // Matches a subtype
        var subtypes = source.scoreranges
            ? Object.keys(source.scoreranges)
            : [];
...
function featureAllowed(source, feature, options) {
    var allowed = true;
    if (options.stacks)
        allowed = allowed && featureMatchesStacks(feature, options);
    if (options.types)
        allowed = allowed && featureMatchesTypes(source, feature, options);
    if (options.languageMode)
        allowed = allowed && featureMatchesLanguage(feature, options);
    return allowed;
}
...
function featureMatchesStacks(feature, options) {
    ...
    return (
        options.stacks.indexOf(feature.properties['carmen:geocoder_stack']) !==
        -1
    );
}
...
function featureMatchesTypes(source, feature, options) {
    for (var i = 0; i < options.types.length; i++) {
        ...
        // Subtype check
        if (
            feature.properties['carmen:types'].indexOf(type[0]) !== -1 &&
            ...
            if (
                feature.properties['carmen:score'] >= range[0] &&
...
function featureMatchesLanguage(feature, options) {
    ...
    var label = closestLang.closestLangLabel(
        options.language[0],
        feature.properties,
        'carmen:text_',
    ...
    return (
        a &&
        b &&
        (a === 'universal' || a === b || equivalentLanguages(label, b))
    );
}
