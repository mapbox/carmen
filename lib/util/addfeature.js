...
function _addFeature(source, input, vtile_only, callback) {
    ...
    } else if (input.type && input.type !== 'Feature') {
        return callback('input must be Feature or FeatureCollection');
    } else {
        input.type = 'Feature';
        input = [ input ];
    }

    for (var feat_it = 0; feat_it < input.features; feat_it++) {
        if (!input[feat_it].properties['carmen:zxy']) {
            input[feat_it].properties['carmen:zxy'] = cover.tiles(input[feat_it].geometry, {min_zoom: source.maxzoom, max_zoom: source.maxzoom}).map(function(zxy) {
                return zxy[2] + '/' + zxy[0] + '/' + zxy[1]
            });

            var zxys = zxyArray(input[feat_it]);
        } else if (!input[feat_it].geometry) {
            var zxys = zxyArray(input[feat_it]);
        ...
        input[feat_it].geometry = input[feat_it].geometry || {
            type: 'Point',
            coordinates: input[feat_it].properties['carmen:center']
        };
    }
