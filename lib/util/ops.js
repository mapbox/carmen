...
function toFeature(context, format, language, debug) {
    var feat = context[0];
    // `carmen:center` property is redundant for point geom, so don't require it
    if ((!feat.geometry || feat.geometry.type !== 'Point') && !feat.properties['carmen:center']&& !feat.properties['carmen:legacy']) throw new Error('Feature has no carmen:center');
    ...
    if (feat.geometry && feat.geometry.type === 'Point') {
        feature.center = feat.geometry.coordinates;
        feature.geometry = feat.geometry;
    } else if (feat.properties['carmen:center']) {
        feature.center = feat.properties['carmen:center'];
        feature.geometry = feat.geometry || {
            type: 'Point',
