    ...
    if (languageFormat) {
        for (var k = 0; k < context.length; k++) {
            var languageText = language
                ? closestLang(language, context[k].properties, 'carmen:text_')
                : false;
            if (languageText || context[k].properties.language === language) {
...
function getPlaceName(context, formatString, language, languageMode, matched) {
    ...
    if (!formatString || formatString === true || formatString === 1) {
        place_name = ((feat.properties['carmen:address']
            ...
            context
                ...
                .map(function(f) {
                    return (
                        (matched ? f.matching_text : undefined) ||
                        closestLang.getText(language, f.properties).text
                    );
                })
                .join(', ')
        ).trim();
        ...
        for (var i = 0; i < template.length; i++) {
            ...
            for (var x = 0; x < context.length; x++) {
                var f = context[x];
                if (!f.properties['carmen:text'])
                    throw new Error('Feature has no carmen:text');
                if (
                    !featureMatchesLanguage(f, {
                        language: [language],
                        languageMode: languageMode
                ...
                if (templateProp == carmenProp) {
                    if (templateSubprop == '_name') {
                        ...
                        val =
                            matched && f.matching_text
                                ? f.matching_text
                                : closestLang.getText(language, f.properties)
                                      .text;
...
function toFeature(
    ...
    if (!feat.properties['carmen:center'])
        throw new Error('Feature has no carmen:center');
    if (!feat.properties['carmen:extid'])
        throw new Error('Feature has no carmen:extid');
    ...
    if (feat.matching_language)
        feature.matching_language = feat.matching_language;
    ...
    languages.reduce(function(memo, language, i) {
        ...
        if (text.language)
            memo[`language${suffix}`] = text.language.replace('_', '-');
        memo[`place_name${suffix}`] = getPlaceName(
            context,
            formatString,
            language,
            languageMode
        ...
        if (i === 0) {
            ...
            if (geocoder) {
                for (var k = 0; k < context.length; k++) {
                    var matched =
                        !!context[k].properties['carmen:query_text'] &&
                        getMatchingText(context[k], geocoder, language);
                    if (matched) {
                        context[k].matching_text = matched.matching_text;
                        context[k].matching_language =
                            matched.matching_language;
                        if (k === 0)
                            feature.matching_text = matched.matching_text;
                        feature.matching_place_name = getPlaceName(
                            context,
                            formatString,
                            language,
                            languageMode,
    ...
    if (feat.bbox)
        feature.bbox = clipBBox ? bbox.clipBBox(feat.bbox) : feat.bbox;
    ...
    if (feat.properties['carmen:address'])
        feature.address = feat.properties['carmen:address'];
    for (var key in context[0].properties)
        if (key.indexOf('carmen:') !== 0 || debug) {
            if (key === 'id') continue;
    ...
    if (context.length > 1) {
        ...
        for (var c = 1; c < context.length; c++) {
            if (!context[c].properties['carmen:extid'])
                throw new Error('Feature has no carmen:extid');
            if (
                !featureMatchesLanguage(context[c], {
                    language: languages,
                    languageMode: languageMode
                })
            )
            ...
            for (var j = 0; j < propertyKeys.length; j++)
                contextFeat[propertyKeys[j]] =
                    context[c].properties[propertyKeys[j]];
            ...
            languages.reduce(function(memo, language, i) {
                ...
                if (i === 0) {
                    memo.text = memo[`text${suffix}`];
                    if (text.language)
                        memo.language = memo[`language${suffix}`];
                }
                return memo;
            }, contextFeat);
...
function getMatchingText(item, geocoder, requestedLanguage) {
    var closest = requestedLanguage
        ? closestLang.closestLangLabel(
              requestedLanguage,
              item.properties,
              'carmen:text_'
          )
    ...
    if (
        item.properties['carmen:matches_language'] &&
        !/,/.exec(item.properties[textKey])
    )
        return;
    ...
    for (var key of Object.keys(item.properties)) {
        ...
        item.properties[key].split(',').forEach(function(sourceText) {
            ...
            if (texts.length) {
                var phrases = termops.getIndexablePhrases(texts[0].tokens, {
                    __COUNT__: [1]
                });
                phrases.forEach(function(phrase) {
                    if (!allPhrases.has(phrase.phrase))
                        allPhrases.set(phrase.phrase, new Set());
    ...
    var regex = new RegExp(
        '^' + query + (item.properties['carmen:prefix'] ? '' : '$')
    );
    ...
    for (var candidate of allPhrases.keys()) {
        if (regex.exec(candidate)) {
            for (var phraseKey of allPhrases.get(candidate)) {
                if (!matches.hasOwnProperty(phraseKey))
                    matches[phraseKey] = candidate;
            }
        }
    }
    ...
    if (requestedLanguage) {
        best = closestLang.closestLangLabel(
            requestedLanguage,
            matches,
            'carmen:text_'
        );
    ...
    if (!best) {
        ...
        if (matches['carmen:text'] && Object.keys(matches).length == 1) {
            ...
            best = Object.keys(matches)
                .filter(function(x) {
                    return x != 'carmen:text';
                })
                .sort()[0];
        }
    }
    ...
    var matching_language =
        best == 'carmen:text' ? null : best.replace('carmen:text_', '');
    var match_key = matches[best] + '/' + best;
    var matching_set = original.has(match_key)
        ? Array.from(original.get(match_key))
        : [];
