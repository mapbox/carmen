...
function standardize(feat, k) {
    ...
    if (
        ...
        feat.geometry.geometries[k].type !== 'MultiLineString'
    )
        return ranges;

    var stdFeat = {
        lines: feat.geometry.geometries[k].coordinates,
        lf: feat.properties['carmen:lfromhn']
            ? feat.properties['carmen:lfromhn'][k]
            : [],
        lt: feat.properties['carmen:ltohn']
            ? feat.properties['carmen:ltohn'][k]
            : [],
        rf: feat.properties['carmen:rfromhn']
            ? feat.properties['carmen:rfromhn'][k]
            : [],
        rt: feat.properties['carmen:rtohn']
            ? feat.properties['carmen:rtohn'][k]
            : [],
        lp: feat.properties['carmen:parityl']
            ? feat.properties['carmen:parityl'][k]
            : [],
        rp: feat.properties['carmen:parityr']
            ? feat.properties['carmen:parityr'][k]
            : []
    };
        ...
        while (i--) {
            ...
            if (
                ...
                for (var j = 0; j < 4; j++) {
                    ...
                    if (
                        Math.abs(address - nearest.hn) <
                        Math.abs(address - hns[j])
                    )
                        continue;
                    nearest.hn = hns[j];
...
function reverse(feat, query) {
    ...
    for (var f_it = 0; f_it < feat.geometry.geometries.length; f_it++) {
        ...
        while (i--) {
            ...
            if (
                !potential ||
                current.pt.properties.dist < potential.pt.properties.dist
            ) {
                potential = current;
    ...
    var num;
    if (sideBinary === leftSideBinary) {
        //If point is on the centre
        num = matchSide(potentialRange, 'left', potential);
    } else {
        //If point is on the right or on the line
...
function matchSide(range, side, point, strict) {
    ...
    if (range && !range[sideVar.parity] && !strict)
        return matchSide(range, opp, point, true);
...
function det2D(start, end, query) {
    return (
        (end[0] - start[0]) * (query[1] - start[1]) -
        (end[1] - start[1]) * (query[0] - start[0])
    );
}
function sign(num) {
    return typeof num === 'number'
        ? num ? (num < 0 ? -1 : 1) : num === num ? 0 : NaN
        : NaN;
}
...
function getReversePoint(pt, coords, units) {
    ...
    for (i = 0; i < coords.length - 1; i++) {
        ...
        var perpendicularPt = turf.destination(
            pt,
            1000,
            direction + 90,
            'miles'
        ...
        if (!intersect) {
            perpendicularPt = turf.destination(
                pt,
                1000,
                direction - 90,
                'miles'
        ...
        if (stop.properties.dist < closestPt.properties.dist) {
            closestPt = stop;
            closestPt.properties.travelled =
                travelled + turf.distance(turf.point(coords[i]), stop, units);
        }
        if (
            intersectPt &&
            ...
            closestPt.properties.travelled =
                travelled +
                turf.distance(turf.point(coords[i]), intersectPt, units);
        }

        closestPt.properties.index = i;
        travelled += turf.distance(
            turf.point(coords[i]),
