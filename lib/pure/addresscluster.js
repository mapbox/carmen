...
module.exports.reverse = function(feat, query) {
    if (feat.geometry.type !== 'MultiLineString') return false;
    ...
    for (var cluster_it = 0; cluster_it < cluster.length; cluster_it++) {
        ...
        var l = cluster[cluster_it].length;
        while (l--) {

            var lon2 = feat.geometry.coordinates[cluster_it][l][0],
                lat2 = feat.geometry.coordinates[cluster_it][l][1];
            ...
            if (closest === undefined || closest.distance > dist) {
                closest = {cluster_it: cluster_it, cluster_pos: l, distance: dist, address: cluster[cluster_it][l]};
            }
        }
    }
    ...
    feat.geometry = {
        type: 'Point',
        coordinates: feat.geometry.coordinates[closest.cluster_it][closest.cluster_pos]
    }
