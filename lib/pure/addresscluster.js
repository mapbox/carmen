...
function forward(feature, address) {
    ...
    for (var c_it = 0; c_it < cluster.length; c_it++) {
        ...
        var a_index = cluster[c_it].reduce(
            (a, e, i) => (e === a1 ? a.concat(i) : a),
            []
        );
        if (!a_index.length)
            a_index = cluster[c_it].reduce(
                (a, e, i) => (e === a2 ? a.concat(i) : a),
                []
            );
        ...
        if (
            a_index.length &&
            feature.geometry.geometries[c_it].type === 'MultiPoint'
        ) {
            return a_index.map(idx => {
                return {
                    ...
                    coordinates: [
                        Math.round(
                            feature.geometry.geometries[c_it].coordinates[
                                idx
                            ][0] * 1e6
                        ) / 1e6,
                        Math.round(
...
function reverse(feat, query) {
    ...
    feat.geometry = {
        type: 'Point',
        coordinates:
            feat.geometry.geometries[closest.cluster_it].coordinates[
                closest.cluster_pos
            ]
    };
