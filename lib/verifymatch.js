...
function verifyFeatures(query, geocoder, spatial, loaded, options) {
    ...
    for (var pos = 0; pos < loaded.length; pos++) {
        ...
        if (checks) {
            ...
            feat.properties["carmen:distance"] = feat.geometry !== undefined ?
                proximity.distanceToGeometry(options.proximity, feat.geometry) :
                proximity.distance(options.proximity, feat.properties["carmen:center"]);
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        if (options.proximity && feat.properties["carmen:score"] >= 0) {
            feat.properties["carmen:scoredist"] = Math.max(
                ...
                proximity.scoredist(feat.properties["carmen:distance"], meanScore)
            );
        } else {
