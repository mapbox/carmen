...
function verifyContext(query, context, strict, loose, groups) {
    ...
    var relevance = 0;
    var garbage = 0;
    var matchingIndices = Object.keys(strict).length;

    for (var q = 0; q < query.length; q++) {
        if (garbage) break;
        var tokenPosition = (1<<q);
        for (var c = 0; c < context.length; c++) {
            var matched = strict[context[c].properties["carmen:tmpid"]] || loose[context[c].properties["carmen:tmpid"]];
            if (!matched) continue;
            else if (!(tokenPosition & matched.mask)) {
                garbage = 1;
                break;
            }
        }
    ...
    var tokenWeight = 1/(matchingIndices + garbage);
