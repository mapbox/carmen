...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    if (options.types || options.stacks) {
        spatialmatches = spatialmatches.filter(function(spatialmatch) {
            ...
            if (!source)
                console.error(
                    new Error(
                        'Misuse: source undefined for idx ' + firstCover.idx
                    )
                );
    ...
    if (spatialmatches.length > 20)
        spatialmatches = spatialmatches.slice(0, 20);

    var q = queue(10);
    for (var i = 0; i < spatialmatches.length; i++)
        q.defer(loadFeature, spatialmatches[i], i);
    ...
    function loadFeature(spatialmatch, pos, callback) {
        ...
        if (!source)
            return callback(
                new Error('Misuse: source undefined for idx ' + cover.idx)
            );
    ...
    function afterFeatures(err, loaded) {
        ...
        if (
            ...
            loaded.forEach(function(feature, i) {
                ...
                var source =
                    geocoder.indexes[feature.properties['carmen:index']];
            ...
            verified = verifyFeatures(
                query,
                geocoder,
                filteredSpatialmatches,
                filteredLoaded,
                options
            );
...
function verifyFeatures(query, geocoder, spatialmatches, loaded, options) {
    ...
    for (var pos = 0; pos < loaded.length; pos++) {
        ...
        if (source.geocoder_address) {
            if (
                address &&
                (feats[0].properties['carmen:addressnumber'] ||
                    feats[0].properties['carmen:rangetype'])
            ) {
                feats[0].properties['carmen:address'] = address.addr;

                var addressPoints = [];
                if (feats[0].properties['carmen:addressnumber']) {
                    addressPoints = addressCluster.forward(
                        feats[0],
                        address.addr
                    );
                }
        ...
        for (let feat of feats) {
            if (
                options.bbox &&
                !bbox.inside(feat.properties['carmen:center'], options.bbox)
            )
                continue;
            ...
            feat.properties['carmen:distance'] = proximity.distance(
                options.proximity,
                feat.properties['carmen:center'],
                cover
            );
    ...
    for (var i = 0; i < result.length; i++) {
        let feat = result[i];
        var languageText = options.language
            ? closestLang(options.language[0], feat.properties, 'carmen:text_')
            : false;
    ...
    for (var a = 0; a < contexts.length; a++) {
        ...
        for (var ctx of context) {
            if (verify[ctx.properties['carmen:tmpid']]) {
                ...
                ctx.properties['carmen:matches_language'] =
                    cover.matches_language;
...
function verifyContext(context, peers, strict, loose, indexes) {
    ...
    for (var c = 0; c < context.length; c++) {
        ...
        var matched =
            strict[feat.properties['carmen:tmpid']] ||
            loose[feat.properties['carmen:tmpid']];
        if (!matched) continue;
        ...
        if (
            ...
            squishy += Math.max(
                peers[feat.properties['carmen:tmpid']].properties[
                    'carmen:score'
                ] || 0,
                0
            );
    ...
    if (squishy > 0) context[0].properties['carmen:scoredist'] += squishy;
...
function sortFeature(a, b) {
    return (
        b.properties['carmen:spatialmatch'].relev -
            a.properties['carmen:spatialmatch'].relev ||
        (a.properties['carmen:address'] === null ? 1 : 0) -
            (b.properties['carmen:address'] === null ? 1 : 0) ||
        (a.geometry && a.geometry.omitted ? 1 : 0) -
            (b.geometry && b.geometry.omitted ? 1 : 0) ||
        (b.properties['carmen:scoredist'] || 0) -
            (a.properties['carmen:scoredist'] || 0) ||
        (a.properties['carmen:position'] || 0) -
            (b.properties['carmen:position'] || 0) ||
        0
    );
}
...
function sortContext(a, b) {
    ...
    if (
        a[0].properties['carmen:address'] &&
        b[0].properties['carmen:address']
    ) {
        // Prefer addresses with an earlier position in the query
        // so if you have a query "70 St. #501" it will prioritize 70
        if (
            a[0].properties['carmen:addresspos'] <
            b[0].properties['carmen:addresspos']
        )
            return -1;
        if (
            b[0].properties['carmen:addresspos'] <
        ...
        if (
            a[0].properties['carmen:addressnumber'] &&
            !b[0].properties['carmen:addressnumber']
        )
            return -1;
        if (
            b[0].properties['carmen:addressnumber'] &&
    ...
    // omitted difference
    var omitted =
        (a[0].geometry && a[0].geometry.omitted ? 1 : 0) -
        (b[0].geometry && b[0].geometry.omitted ? 1 : 0);
    if (omitted !== 0) return omitted;

    // sort by spatialmatch position ("stable sort")
    if (a[0].properties['carmen:position'] < b[0].properties['carmen:position'])
        return -1;
    if (a[0].properties['carmen:position'] > b[0].properties['carmen:position'])
        return 1;
