    ...
    function afterFeatures(err, loaded) {
        ...
        if (options.scoreAbove || options.stacks) {
            ...
            for (var j = 0; j < loaded.length; j++) {
                ...
                if (qualifyResult([options.stacks, options.scoreAbove], [{func: stackMatch, arguments: [loaded[j].properties['carmen:geocoder_stack'], options.stacks]}])) {
...
function dropFeature(geocoder, options, results, callback) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        function scoreMatch(optionScores, resultScore) {
            // console.log('resultScore ', resultScore, ' optionScores ', optionScores);
            return (resultScore >= optionScores);
        }
        if (qualifyResult([options.types, options.stacks, options.scoreAbove], [{func: typeMatch, arguments: [options.types, source.type]}, {func: stackMatch, arguments: [options.stacks, source.stack]}, {func: scoreMatch, arguments: [options.scoreAbove * results[i][0].scorefactor, results[i][0].score]}])) {
