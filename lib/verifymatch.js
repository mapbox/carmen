...
module.exports.verifyContext = verifyContext;
...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function afterFeatures(err, loaded) {
        ...
        loadContexts(geocoder, verified, sets, options, afterContexts);
    }

    function afterContexts(err, contexts) {
        if (err) return callback(err);
        return callback(null, verifyContexts(query, contexts, sets, geocoder.names));
    }
}
...
function verifyContexts(query, contexts, sets, groups) {
    for (var a = 0; a < contexts.length; a++) {
        ...
        var strictRelev = verifyContext(query, context, verify, {}, groups);
        var looseRelev = verifyContext(query, context, verify, sets, groups);
...
function verifyContext(query, context, strict, loose, groups) {
    ...
    var relevance = 0;
    var garbage = 0;
    var strictKeys = Object.keys(strict);
    var strictMask = strict[strictKeys[0]].mask;

    for (var s = 1; s < strictKeys.length; s++) {
        strictMask = strictMask | (strict[Object.keys(strict)[s]].mask);
    }

    if (query.length !== (strictMask.toString(2).split('1').length - 1)) {
        garbage = 1;
    }

    var tokenWeight = 1/(strictKeys.length + garbage);
    ...
    for (var c = 0; c < context.length; c++) {
        ...
        if (backy) {
            relevance += tokenWeight * 0.5;
        } else {
            relevance += tokenWeight;
        }
    }
    relevance -= 0.01;
