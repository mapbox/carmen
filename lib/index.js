...
function generateFrequency(docs, replacer) {
    ...
    for (var i = 0; i < docs.length; i++) {
        if (docs[i].type === 'FeatureCollection') {
            var doc = docs[i].features[0];
        } else {
            var doc = docs[i];
        }

        if (!doc.properties["carmen:text"]) {
            throw new Error('doc has no carmen:text');
        }

        // set max score
        freq[1][0] = Math.max(freq[1][0], doc.properties["carmen:score"] || 0);

        var texts = termops.getIndexableText(replacer, doc);
