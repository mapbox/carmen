...
function generateFrequency(docs, replacer) {
    ...
    for (var i = 0; i < docs.length; i++) {
        if (docs[i].type === 'FeatureCollection') {
            var text = docs[i].features[0].properties['carmen:text'];
            var score = docs[i].features[0].properties['carmen:score'];
            for (var feat_it = 1; feat_it < docs[i].features.length; feat_it++) {
                score = Math.max(score, docs[i].features[feat_it].properties['carmen:score']);
                if (text !== docs[i].features[feat_it].properties['carmen:text']) {
                    throw new Error('featurecollection must have matching carmen:text');
                }
            }
        } else {
            var text = docs[i].properties["carmen:text"];
            var score = docs[i].properties["carmen:score"];
        }
        ...
        // set max score
        freq[1][0] = Math.max(freq[1][0], score || 0);
