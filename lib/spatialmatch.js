...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    function coalesceStacks() {
        var q = queue();
        for (var i = 0; i < stacks.length; i++)
            q.defer(coalesceStack, stacks[i]);
    ...
    function coalesceStack(stack, callback) {
        ...
        if (options) {
            ...
            if (options.bbox) {
                coalesceOpts.bboxzxy = bbox.insideTile(
                    options.bbox,
                    stack[0].zoom
                );
            }
        ...
        coalesce(stack, coalesceOpts, function(err, cacheSpatialmatches) {
            ...
            for (var i = 0; i < cacheSpatialmatches.length; i++) {
                spatialmatches.push(
                    new Spatialmatch(cacheSpatialmatches[i], byIdx)
                );
            }
    ...
    function coalesceFinalize(err, results) {
        ...
        for (var i = 0; i < combined.length; i++) {
            ...
            for (var j = 0; j < covers.length; j++) {
                if (
                    !sets[covers[j].tmpid] ||
                    sets[covers[j].tmpid].relev < covers[j].relev
                )
            ...
            if (
                covers.length > 1 &&
                covers[0].idx > covers[1].idx &&
                !doneDescending[covers[0].tmpid]
            ) {
        ...
        return callback(null, {
            results: filteredSpatialmatches,
            sets: sets,
            waste: waste
        });
    }
...
function rebalance(query, stack) {
    ...
    var garbage =
        query.length === stackMask.toString(2).split(1).length - 1 ? 0 : 1;
...
function Cover(cacheCover, phrasematch) {
    ...
    this.score = termops.decode3BitLogScale(
        cacheCover.score,
        phrasematch.scorefactor
    );
    this.scoredist =
        cacheCover.scoredist > 7
            ? phrasematch.scorefactor / 7 * cacheCover.scoredist
            : termops.decode3BitLogScale(
                  cacheCover.scoredist,
                  phrasematch.scorefactor
              );
