...
function spatialmatch(query, phrasematches, options, geocoder, callback) {
    var stacks = stackable(phrasematches);

    //Filter forward geocodes by type
    if (options.types) {
        var filtered = [];
        for (var i = 0; i < stacks.length; i++) {
            var type = geocoder.byidx[stacks[i][0].idx]._geocoder.name;
