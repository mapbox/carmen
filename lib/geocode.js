...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.stacks) {
        if (!Array.isArray(options.stacks) || options.stacks.length < 1)
            return callback(errcode('options.stacks must be an array with at least 1 stack', 'EINVALIDSTACK'));
        var l = options.stacks.length;
        while (l--) if (!geocoder.bystack[options.stacks[l]])
            return callback(errcode('Stack "' + options.stacks[l] + '" is not a known stack. Must be one of: ' + Object.keys(geocoder.bystack).join(', '), 'EINVALIDSTACK', {validStacks: Object.keys(geocoder.bystack)}));
    }
