...
module.exports = function(geocoder, query, options, callback) {
    ...
    options.autocomplete =
        options.autocomplete === undefined ? true : options.autocomplete;
    ...
    if (options.types) {
        if (!Array.isArray(options.types) || options.types.length < 1)
            return callback(
                errcode(
                    'options.types must be an array with at least 1 type',
                    'EINVALID'
                )
            );

        var l = options.types.length;
        var acceptableTypes = Object.keys(geocoder.bytype).concat(
            Object.keys(geocoder.bysubtype)
        );
        while (l--) {
            if (acceptableTypes.indexOf(options.types[l]) === -1) {
                return callback(
                    errcode(
                        'Type "' +
                            options.types[l] +
                            '" is not a known type. Must be one of: ' +
                ...
                var subtypeAndTypeExist =
                    geocoder.bytype[options.types[l]] &&
                    options.types.toString().indexOf(options.types[l]) != l;
                var subtypeIndexWhenTypeExists = subtypeAndTypeExist
                    ? options.types.toString().indexOf(options.types[l])
                    : -1;
    ...
    if (options.stacks) {
        if (!Array.isArray(options.stacks) || options.stacks.length < 1)
            return callback(
                errcode(
                    'options.stacks must be an array with at least 1 stack',
                    'EINVALID'
                )
            );
            ...
            if (!geocoder.bystack[options.stacks[k]])
                return callback(
                    errcode(
                        'Stack "' +
                            options.stacks[k] +
                            '" is not a known stack. Must be one of: ' +
                            Object.keys(geocoder.bystack).join(', '),
                        'EINVALID'
                    )
                );
        }
    ...
    if (options.proximity) {
        if (
            !options.proximity instanceof Array ||
            options.proximity.length !== 2
        )
            return callback(
                errcode(
                    'Proximity must be an array in the form [lon, lat]',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.proximity[0]) ||
            options.proximity[0] < -180 ||
            options.proximity[0] > 180
        )
            return callback(
                errcode(
                    'Proximity lon value must be a number between -180 and 180',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.proximity[1]) ||
            options.proximity[1] < -90 ||
            options.proximity[1] > 90
        )
            return callback(
                errcode(
                    'Proximity lat value must be a number between -90 and 90',
                    'EINVALID'
                )
            );
    }
    ...
    if (options.language) {
        options.language = Array.isArray(options.language)
            ? options.language
        ...
        if (options.language.length > 20)
            return callback(
                errcode(
                    'options.language should be a list of no more than 20 languages',
                    'EINVALID'
                )
            );
        if (uniq(options.language).length != options.language.length)
            return callback(
                errcode(
                    'options.language should be a list of unique language codes',
                    'EINVALID'
                )
            );
    }
    ...
    if (options.languageMode) {
        if (options.languageMode !== 'strict')
            return callback(
                errcode(
                    "'" +
                        options.languageMode +
                        "' is not a valid language mode",
                    'EINVALID'
                )
            );
    }
    ...
    if (options.bbox) {
        ...
        if (!options.bbox instanceof Array || options.bbox.length !== 4)
            return callback(
                errcode(
                    'BBox is not valid. Must be an array of format [minX, minY, maxX, maxY]',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.bbox[0]) ||
            options.bbox[0] < -180 ||
            options.bbox[0] > 180
        )
            return callback(
                errcode(
                    'BBox minX value must be a number between -180 and 180',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.bbox[1]) ||
            options.bbox[1] < -90 ||
            options.bbox[1] > 90
        )
            return callback(
                errcode(
                    'BBox minY value must be a number between -90 and 90',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.bbox[2]) ||
            options.bbox[2] < -180 ||
            options.bbox[2] > 180
        )
            return callback(
                errcode(
                    'BBox maxX value must be a number between -180 and 180',
                    'EINVALID'
                )
            );
        if (
            isNaN(options.bbox[3]) ||
            options.bbox[3] < -90 ||
            options.bbox[3] > 90
        )
            return callback(
                errcode(
                    'BBox maxY value must be a number between -90 and 90',
                    'EINVALID'
                )
            );
        if (options.bbox[0] > options.bbox[2])
            return callback(
                errcode(
                    'BBox minX value cannot be greater than maxX value',
                    'EINVALID'
                )
            );
        if (options.bbox[1] > options.bbox[3])
            return callback(
                errcode(
                    'BBox minY value cannot be greater than maxY value',
                    'EINVALID'
                )
            );
    }
    ...
    if (options.reverseMode) {
        if (
            options.reverseMode !== 'score' &&
            options.reverseMode !== 'distance'
        )
            return callback(
                errcode(
                    options.reverseMode +
                        ' is not a valid reverseMode. Must be one of: score, distance',
                    'EINVALID'
                )
            );
    }
...
function reverseGeocode(geocoder, tokenized, options, callback) {
    ...
    var parentTypes = [];
    if (options.types)
        parentTypes = options.types.map(function(t) {
            return t.split('.')[0];
        });
    ...
    for (var type in geocoder.bytype) {
        if (options.types && parentTypes.indexOf(type) === -1) continue;
            ...
            function(err, feats) {
                ...
                q.awaitAll(function(err, contexts) {
                    ...
                    for (
                        ...
                        if (
                            feat.properties &&
                            feat.properties['carmen:tmpid']
                        ) {
                            var dedupeId =
                                feat.properties['carmen:tmpid'] +
                                '-' +
                                (feat.properties['carmen:address'] || '');
                            if (observedIds.indexOf(dedupeId) !== -1) continue;
...
function forwardGeocode(geocoder, query, options, callback) {
    options.limit = options.limit
        ? options.limit > 10 ? 10 : options.limit
        : 5;
    ...
    for (var dbid in geocoder.indexes)
    ...
    q.awaitAll(function(err, phrasematchResults) {
        ...
        if (options.debug) {
            ...
            for (var idx = 0; idx < phrasematchResults.length; idx++) {
                ...
                for (
                    var x = 0;
                    x < phrasematchResults[idx].phrasematches.length;
                    x++
                ) {
    ...
    function spatialmatchComplete(err, matched) {
        ...
        if (matched.waste && matched.waste.length) {
            queryData.waste = matched.waste.map(function(idxSet) {
                return idxSet.map(function(idx) {
                    return geocoder.byidx[idx].id;
                });
            });
        }
