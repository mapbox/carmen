...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    var tokenized = termops.tokenize(
        token.replaceToken(source.token_replacer, query)
    );
    ...
    if (options.bbox) {
        ...
        if (!intersects)
            return callback(null, new PhrasematchResult([], source));
    }
    ...
    if (source.geocoder_address) {
        ...
        for (var i = 0; i < numTokenized.length; i++) {
            subqueries = subqueries.concat(
                termops.permutations(numTokenized[i])
            );
        }
    }
    ...
    while (l--) {
        ...
        if (text) {
            ...
            var tryNormalization =
                source.use_normalization_cache &&
                (!scanPrefix || text.length >= 3);
            ...
            if (!tryNormalization) {
                ...
                variants = dawgMatch.normalizations.length
                    ? dawgMatch.normalizations.map(n => {
                          return { text: n, prefix: scanPrefix };
                      })
                    : [{ text: subquery.join(' '), prefix: scanPrefix }];
            }
            ...
            for (let variant of variants) {
                ...
                var prefix =
                    variant.prefix &&
                    !(source.geocoder_address && termops.isAddressNumber(text));
                ...
                languages = languages.map(function(l) {
                    ...
                    if (source.lang.lang_map.hasOwnProperty(l)) {
                        ...
                        var label = cl.closestLangLabel(
                            l,
                            source.lang.lang_map
                        );
